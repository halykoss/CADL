...
exp ::=  Var(name) | Num(int) | Fun(name, t, exp) 
...
v ::= TypInnerTup(t,v) | End.
t ::=  TypI | TypUnit |TypF(t, t) | TypTu(v) |TypList(t) | TypBool | TypRef(t).
@PrintType.
...
type_check(C, Var(x), T) :- member(C, x, T).
type_check(C, Fun(x, t1, e), TypF(t1,t2)) :- add(C, x, t1, C1), type_check(C1, e, t2).
type_check(C, App(e1, e2), t2) :- type_check(C, e1, TypF(t1, t2)), type_check(C, e2, t3), @Compat(t1,t3).
type_check(C, Let(x,e,e1),t2) :- type_check(C,e,t1), add(C, x, t1 , C1), type_check(C1, e1, t2).
